...
#driver start times adjusted for respective local time zones with Daylight Savings
#time accounted for
import json
import pandas as pd
import requests
...
#requests today's data from the api
def new_json(date):
    headers = {
...
#reading through the json files to find the driving start time and
#return as a df
...
def main():
    ...
    monday_list=[i for i in date_list if i.day_name() == 'Monday']
